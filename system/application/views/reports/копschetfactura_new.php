...
<tr align="center">
	...
	<?php
		...
		{
			echo (f_d($s2->tenge*12/100*1000)/1000);$sumnds+=(f_d($s2->tenge*12/100*1000)/1000);
			$sum_nds+=$s2->nds_value;
			$i_nds-=f_d($s2->nds_value);
		}
		else
			{echo f_d3($posl*12/100*1000)/1000;$sumnds=(f_d($posl*12/100))+$sumnds;}
	?>
	</td>
	...
	<?php 
		...
		{
			echo f_d($s2->tenge*1000+$s2->tenge*12/100*1000)/1000;$sumitog+=f_d($s2->tenge*1000+$s2->tenge*12/100*1000)/1000;
			$sum+=$s2->with_nds;
