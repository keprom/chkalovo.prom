...
if (strlen($data_schet)==0){ echo datetostring($schetfactura_date->date);} else { echo $data_schet;}?>
 ...
 <tr>
 <td width=2100px>Жүк жіберуші<br/> Грузоотправитель <u><?php if (strlen($edit5)==0) { echo "БИН ".$org->bin.", ".$org->org_name.", ".$org->address;} else {echo $edit5;}?>                                                                                                                   </u></td>
 <td> (8)</td>
 </tr>
 <tr align=right>
 ...
 <tr>
 <td width=2100px>Жүк алушы БСН<br/>Грузополучатель БИН  <u>  <?php if (strlen($edit6)==0) {echo $firm->bin.", ".$firm->name.", ".$firm->address;} else {echo $edit6;}?>                                                                                                                 </u>   </td>
 <td> (9)</td>
 </tr>
 <tr align=right>
...
<tr align="center">
	...
	<TD width="400px">Услуги по передаче и/или распределению электроэнергии за <br/> <?php echo datetostring2($schetfactura_date->date);?> года </td>
	...
	<?php
		...
		{
			echo floor(f_d($s2->tenge*12/1))/100;$sumnds+=(f_d($s2->tenge*12/100*1000)/1000);
			$sum_nds+=$s2->nds_value;
			$i_nds-=f_d($s2->nds_value);
		}
		else
			{echo f_d($i_nds);$sumnds=(f_d($posl*12/100))+$sumnds;}
	?>
	</td>
	...
	<?php 
		...
		{
			echo floor(f_d($s2->tenge*100+$s2->tenge*12/1))/100;floor($sumitog+=f_d($s2->tenge*100+$s2->tenge*12/1))/100;
			$sum+=$s2->with_nds;
			$i_itogo-=f_d($s2->with_nds);
		}
		else
		{	echo f_d($i_itogo);$sumitog=$itog->itogo_with_nds;}
	?>
	</td>
	<TD width="110px">0</td>
	<TD width="100px">0</TD>
...
<br />
 ...
 <tr>
 <td align="left"><b>Ответственный на подписание<br/>за Руководителя:</b> <?php echo "Зам.Директора "; ?> </td>
 <td></td>
 <td align="right"></td>
 </tr>
  <tr>
...
<tr>
 ...
 <tr>
 <td align="left"><b>Ответственный на подписание<br/>за Гл.Бухгалтера:</b> <?php echo "Зам.Гл.Бухгалтера Осанова Ж.Ш. "; ?> </td>
 <td></td>
 <td align="right"></td>
 </tr>
  <tr>
